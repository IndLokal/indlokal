/**
 * Report event content - PRD/TDD-0040.
 * Mirrors the community report sheet for an event by id (passed via params).
 * POST /api/v1/reports with ContentReportInput { eventId }.
 */
...
const REPORT_TYPES: Array<{ value: r.ReportType; label: string; help: string }> = [
  { value: 'STALE_INFO', label: 'Out of date', help: 'Date, time, or venue look stale.' },
  { value: 'BROKEN_LINK', label: 'Broken link', help: 'The registration link no longer works.' },
  {
    value: 'INCORRECT_DETAILS',
    label: 'Incorrect details',
    help: 'The event details are wrong.',
  },
  ...
  if (!user) {
    return (
      <SafeAreaView style={styles.safeArea}>
        <Stack.Screen options={{ title: 'Report' }} />
        <View style={styles.center}>
          <Text style={styles.title}>Sign in to report events</Text>
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      >
        <ScrollView contentContainerStyle={styles.container}>
          <Text style={styles.title}>Report this event</Text>
