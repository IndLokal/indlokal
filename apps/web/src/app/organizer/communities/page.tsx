    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted mt-1 text-sm">
          {user.claimedCommunities.length} community workspace
          {user.claimedCommunities.length !== 1 ? 's' : ''}
        </p>
      </div>

      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {user.claimedCommunities.map((c) => {
          ...
          const pct = Math.round((completeness.filter(Boolean).length / completeness.length) * 100);
      ...
      <Link href="/organizer" className="text-muted inline-block text-sm hover:underline">
        ← Back to dashboard
      </Link>
    </div>
  );
